...
const Map = ({ props }) => {
  ...
  const size = calSize(width, height, data1.nRow, data1.nCol) * 1.5;
  let zoom = d3.zoom();
  useEffect(() => {
    navigate("/map?zoom=3");
    let data = data1;
    ...
    function drawImage(scale) {
      for(let i=0; i < data.data.length; i++){
        if(data.data[i].img){
          ...
          var img = new Image();
          let x = data.data[i].position.colStart * size * scale;
          let y = data.data[i].position.rowStart * size * scale;
          let square = data.data[i].position.colEnd - data.data[i].position.colStart + 1;
          img.src = `/${data.data[i].img}`;
          img.onload = function () {
            contextImg.drawImage(img, x, y, square * size * scale, square * size * scale );
          };
        }
      }
